package org.firstinspires.ftc.teamcode.adaptations.odometry;

import static java.lang.Math.PI;
import static java.lang.Math.cos;
import static java.lang.Math.sin;
import static java.lang.Math.toDegrees;
import static java.lang.Math.toRadians;

import android.annotation.SuppressLint;

import androidx.annotation.NonNull;

/** @noinspection unused*/
public class Pose {
    public static boolean HOLD_DEFAULT = true;

    public double x;
    public double y;
    public double heading;
    public boolean hold = HOLD_DEFAULT;

    public Pose(double x, double y, double heading) {
        this.x = x;
        this.y = y;
        this.heading = normalize(heading);
    }

    public Pose(double x, double y, double heading, boolean hold) {
        this.x = x;
        this.y = y;
        this.heading = normalize(heading);
        this.hold = hold;
    }

    public Pose axial(double distance) {
        return new Pose(
            x + cos(heading) * distance,
            y + sin(heading) * distance,
            heading,
            hold
        );
    }

    public Pose lateral(double distance) {
        return new Pose(
            x + cos(heading + PI / 2) * distance,
            y + sin(heading + PI / 2) * distance,
            heading,
            hold
        );
    }

    public Pose midpoint(Pose pose) {
        return new Pose(
            (x + pose.x) / 2,
            (y + pose.y) / 2,
            normalize((heading + pose.heading) / 2)
        );
    }

    public Pose turn(double degrees) {
        return new Pose(
            x, y, normalize(heading + toRadians(degrees)), hold
        );
    }

    public Pose reverse() {
        return reverse(0);
    }

    public Pose reverse(double offset) {
        return turn(180 + offset);
    }

    public Pose face(Pose pose) {
        return face(pose, 0);
    }

    public Pose face(Pose pose, double degrees) {
        return new Pose(x, y, normalize(atan2(pose) + toRadians(degrees)), pose.hold);
    }

    public Pose hold(boolean hold) {
        return new Pose(x, y, heading, hold);
    }

    public double hypot() {
        return Math.hypot(x, y);
    }

    public double hypot(Pose pose) {
        return Math.hypot(
            pose.x - x,
            pose.y - y
        );
    }

    public double atan2(Pose pose) {
        return Math.atan2(pose.y - y, pose.x - x);
    }

    public double normalize(double heading) {
        if (heading > +PI) heading -= PI * 2;
        if (heading < -PI) heading += PI * 2;
        return heading;
    }

    @NonNull
    @SuppressLint("DefaultLocale")
    public String toString() {
        return String.format("%.1fx, %.1fy, %.1f°", x, y, toDegrees(heading));
    }
}
